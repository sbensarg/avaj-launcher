    ...
    public void updateConditions() {
        ...
        try {
            ...
            if (weather.equals("SUN")) {
                latitude += 10;
                height += 2;
                message = String.format("JetPlane#%s(%d): It's a beautiful day, let's fly high!", this.name, this.id);
            } else if (weather.equals("RAIN")) {
                latitude += 5;
                message = String.format("JetPlane#%s(%d): The rain is like a blessing from the sky.", this.name, this.id);
            } else if (weather.equals("FOG")) {
                latitude += 1;
                message = String.format("JetPlane#%s(%d): Foggy days are like slow motion days.", this.name, this.id);
            } else if (weather.equals("SNOW")) {
                height -= 7;
                if (height < 0) {
                    height = 0;
                }
                message = String.format("JetPlane#%s(%d): Winter is coming! Brace yourselves!", this.name, this.id);
            }
